...
export const TROPHIES: TrophyDef[] = [
  ...
  },
  {
    ...
    descripcion: 'Alcanza 1.0 PPS en una sesión', esSecreto: false, valorObjetivo: 1, tipoMeta: 'hito', emoji: '📈',
    evaluate: (s) => {
      const hit = s.some(x => {
        const pts = x.zoneType === '3p' ? x.made * 3 : x.zoneType === '2p' ? x.made * 2 : x.made;
        return x.total > 0 && (pts / x.total) >= 1.0;
      });
      return { unlocked: hit, progress: hit ? 1 : 0, current: hit ? 1 : 0 };
    }
  },
  ...
  },
  {
    ...
    descripcion: 'Promedio 1.1 PPS últimos 30 días', esSecreto: false, valorObjetivo: 1.1, tipoMeta: 'hito', emoji: '💎',
    evaluate: (s) => {
      ...
      return { unlocked: pps >= 1.1, progress: pps >= 1.1 ? 1 : 0, current: Math.round(pps * 100) / 100 };
    }
  },
  {
  ...
  },
  {
    ...
    evaluate: (s) => {
      // Simplification: check if any single FT session has 100% on 100+ shots
      const ft = sessionsByType(s, 'tl');
  ...
  },
  {
    ...
    evaluate: (s) => {
      // This is evaluated externally in the hook
  ...
  // SECRETOS (10)
  {
    id: 'buho_nocturno', nombre: 'Búho Nocturno', saga: 'Horarios', copa: 'secreto',
    descripcion: 'Registra una sesión después de medianoche', esSecreto: true, valorObjetivo: 1, tipoMeta: 'hito', emoji: '🦉',
    evaluate: (s) => {
      const hit = s.some(x => { const h = new Date(x.date).getHours(); return h >= 0 && h < 5; });
      return { unlocked: hit, progress: hit ? 1 : 0, current: hit ? 1 : 0 };
    }
  },
  {
    id: 'madrugador', nombre: 'Madrugador', saga: 'Horarios', copa: 'secreto',
    descripcion: 'Registra una sesión antes de las 7am', esSecreto: true, valorObjetivo: 1, tipoMeta: 'hito', emoji: '🌅',
    evaluate: (s) => {
      const hit = s.some(x => { const h = new Date(x.date).getHours(); return h >= 5 && h < 7; });
      return { unlocked: hit, progress: hit ? 1 : 0, current: hit ? 1 : 0 };
    }
  },
  {
    id: 'hielo_venas', nombre: 'Hielo en las Venas', saga: 'Precisión', copa: 'secreto',
  ...
  {
    id: 'centenario', nombre: 'Centenario', saga: 'Volumen', copa: 'secreto',
  ...
  {
    id: 'perfeccionista', nombre: 'Perfeccionista', saga: 'Precisión', copa: 'secreto',
  ...
  {
    id: 'doble_doble', nombre: 'Doble-Doble', saga: 'Versatilidad', copa: 'secreto',
  ...
  {
    id: 'triple_triple', nombre: 'Triple-Triple', saga: 'Triples', copa: 'secreto',
  ...
  {
    id: 'maraton', nombre: 'Maratón', saga: 'Volumen', copa: 'secreto',
  ...
  {
    id: 'coleccionista', nombre: 'Coleccionista', saga: 'Versatilidad', copa: 'secreto',
  ...
  {
    id: 'fantasma', nombre: 'Fantasma', saga: 'Misterio', copa: 'secreto',
